	...
	setup: function(frm) {
		...
		frm.set_query("reference_doctype", "references", function() {
			if (frm.doc.party_type=="Customer") {
				...
				var doctypes = ["Salary Slip", "Expense Claim", "Journal Entry"];
			} else {
		...
		frm.set_query("reference_name", "references", function(doc, cdt, cdn) {
			...
			party_type_doctypes = ['Sales Invoice', 'Sales Order', 'Purchase Invoice', 'Purchase Order', "Salary Slip", "Expense Claim"];
			...
			callback: function(r, rt) {
				if(r.message) {
					...
					$.each(r.message, function(i, d) {
						...
						if(!in_list(["Sales Order", "Purchase Order", "Salary Slip", "Expense Claim"], d.voucher_type)) {
						...
						if (in_list(['Sales Invoice', 'Purchase Invoice', "Salary Slip"], d.reference_doctype)){
	...
	validate_reference_document: function(frm, row) {
		var _validate = function(i, row) {
			...
			if(frm.doc.party_type=="Employee" &&
				!in_list(["Salary Slip", "Expense Claim"], row.reference_doctype)
			) {
				frappe.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				frappe.msgprint(__("Row #{0}: Reference Document Type must be one of Salary Slip or Expense Claim", [row.idx]));
				return false;
			}
		}
