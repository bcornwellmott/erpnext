...
class PaymentEntry(AccountsController):
	...
	def validate_reference_documents(self):
		if self.party_type == "Customer":
			...
			valid_reference_doctypes = ("Salary Slip", "Expense Claim")
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not frappe.db.exists(d.reference_doctype, d.reference_name):
					...
					if d.reference_doctype in ("Sales Invoice", "Purchase Invoice", "Salary Slip", "Expense Claim"):
						if self.party_type=="Customer":
							ref_party_account = ref_doc.debit_to
						elif self.party_type=="Supplier":
							ref_party_account = ref_doc.credit_to
						elif self.party_type=="Employee":
							ref_party_account = ref_doc.payable_account if d.reference_doctype =="Expense Claim" else frappe.db.get_value("Company",
								{"company_name": ref_doc.company}, "default_payroll_payable_account")
							
						if ref_party_account != self.party_account:
							frappe.throw(_("{0} {1} does not associated with Party Account {2} (Account is {3}). You will need to create a separate payment entry for this payment.")
								.format(d.reference_doctype, d.reference_name, self.party_account, ref_party_account))
	...
	def validate_payment_against_negative_invoice(self):
		if ((self.payment_type=="Pay" and self.party_type=="Customer")
				...
				or (self.payment_type=="Receive" and self.party_type=="Employee")):
	...
	def link_salary_slip(self):
		if self.payment_type in ("Pay") and self.party:
			for d in self.get("references"):
				if d.reference_doctype=="Salary Slip" and d.reference_name:
					doc = frappe.set_value("Salary Slip", d.reference_name,"salary_entry", self.name )
					doc = frappe.set_value("Salary Slip", d.reference_name,"salary_reference_type", "Payment Entry")

	def delink_salary_slip(self):
		if self.payment_type in ("Pay") and self.party:
			for d in self.get("references"):
				if d.reference_doctype=="Salary Slip" and d.reference_name:
					doc = frappe.set_value("Salary Slip", d.reference_name,"salary_entry", "" )
					doc = frappe.set_value("Salary Slip", d.reference_name,"salary_reference_type", "")
...
def get_outstanding_reference_documents(args):
	...
	for d in outstanding_invoices:
		...
		if party_account_currency != company_currency:
			if d.voucher_type in ("Sales Invoice", "Purchase Invoice", "Salary Slip", "Expense Claim"):
...
def get_orders_to_be_billed(posting_date, party_type, party, party_account_currency, company_currency):
	...
	if party_type == "Employee":
		orders = frappe.db.sql("""
			select
				name as voucher_no,
				net_pay as invoice_amount,
				net_pay as outstanding_amount,
...
def get_payment_entry(dt, dn, party_amount=None, bank_account=None, bank_amount=None):
	...
	if dt in ("Sales Invoice", "Sales Order"):
		...
		party_type = "Supplier"
	elif dt in ("Salary Slip", "Expense Claim"):
		party_type = "Employee"
	...
	elif dt in ("Sales Invoice", "Purchase Invoice", "Salary Slip", "Expense Claim"):
